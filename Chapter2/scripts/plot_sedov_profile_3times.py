...
def plot_profile(dens=0.101348, gamma_WendlandC2: float = 1.936492):
    ...
    COL_SIZE, ROW_SIZE = 3, 2
    fig, ax = plt.subplots(
        COL_SIZE, ROW_SIZE, figsize=(9, 13), sharex=True
    )
    ...
    dict_sim = {
        ...
        "../run01_diffusion_new_one_p_with_centre_higres_nocooling_sphere_lowT": "high\_res\_n01\_nocooling",
    }
   
    for sim, (key, value) in enumerate(dict_sim.items()):
        print(sim, key, value)
        for counter, idx in enumerate([17, 17*4, 17*7]):  # 119 4 7
            print(counter, idx)
            with h5.File(f"{key}" + "/output_{:04d}.hdf5".format(idx), "r") as f:
                ...
                v_s = np.insert(v_s, np.size(v_s), [0.0, 0.0,],)
                ...
                # Smoothing length
                gas_h = f["/PartType0/SmoothingLengths"][:] * unit_length_in_cgs / constants["PARSEC_IN_CGS"]
                ...
                # Radial velocity
                v_bin = bin_distribute(r, gas_h * gamma_WendlandC2, v_r * gas_m) / m_bin_full
    ...
    ax[2, 0].legend(loc="upper right", fontsize=LEGEND_SIZE * 0.65)
    ax[2, 1].legend(loc="upper right", fontsize=LEGEND_SIZE * 0.65)


    plt.savefig(
        "./images/sedov_radial_profile_3_times.pdf", bbox_inches="tight", pad_inches=0.1
    )
