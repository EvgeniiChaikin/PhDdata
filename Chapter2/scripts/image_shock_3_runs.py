...
def plot(snp, z_aver_width=10.0 / 1e3, gamma=1.936492, X_H=0.73738788833, FONTSIZE=28):
    ...
    for count, (key, value) in enumerate(dict_sim.items()):
        with h5.File(
            ...
            ej_schene[np.where(ej_schene==0.0)] = np.min(ej_schene[np.where(ej_schene>0.0)])
            ...
            if count==2: 
                ...
                cax1.set_ylabel(
                    "$\\mathrm{log}\\, T_{\\rm K}$ [K]", fontsize=FONTSIZE*1.2, labelpad=20
                )  # , rotation=270
            else:
                cax1.yaxis.set_ticklabels([])
            ...
            if count == 2: 
                cax.yaxis.set_ticklabels(["$-13$", "$-12$", "$-11$", "$-10$", "$-9$"], fontsize=FONTSIZE)
                cax.set_ylabel(
                    "log $n_{\\rm ^{60}Fe} \\rm \\, [cm^{-3}]$", fontsize=FONTSIZE*1.2, labelpad=20
                )
            else:
                cax.yaxis.set_ticklabels([])
