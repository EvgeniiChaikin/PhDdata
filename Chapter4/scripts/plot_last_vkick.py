        ...
        if script_name == sys.argv[0]:
            ...
            for plot in plots:
                ...
                for counter, (key, value) in enumerate(dict_sim.items()):
                    ...
                    vkick = f["/PartType0/LastSNIIKineticFeedbackvkick"][:] * unit_length_in_cgs / unit_time_in_cgs / 1e5
                    ...
                    if split == 0:
                        if counter < len(dict_sim.items()) - 3:
                            line, = ax.plot(10.0 ** centers, values, label=key.replace("_","\_"), lw=3.0)
                            ax.axvline(x=np.median(vkick[mask]),lw=1.5,zorder=-10,
                                                           color=line.get_color(),
                                                           dashes=(6, 3))
                        elif counter == len(dict_sim.items()) - 3:
                            ref1, = ax.plot(10.0 ** centers, values, lw=2.0, color="black", dashes=(8,2,2,2), zorder=1000)
                        elif counter == len(dict_sim.items()) - 2:
                            ref2 = ax.fill_between(10.0 ** centers, np.ones_like(values) / 1e5, values, color='chocolate', 
                                                   alpha=0.6)
                        elif counter == len(dict_sim.items()) - 1:
                            ref3 = ax.fill_between(10.0 ** centers, np.ones_like(values) / 1e5, values, color='grey', 
                                                   alpha=0.2)
                    elif split == 7:
                        line, = ax.plot(10.0 ** centers, values, label=key.replace("_","\_"),
                ...
                ax.set_xlabel("Actual kick velocity $\\Delta v$ [km s$^{-1}$]", fontsize=33)
