        ...
        if script_name == sys.argv[0]:
            ...
            for plot in plots:
                ...
                for counter, (key, value) in enumerate(dict_sim.items()):
                    ...
                    snii_th_feedback_times_stars = (
                        data.stars.last_sniithermal_feedback_times
                    )

                    # Create mask
                    mask = np.logical_and(
                        snii_th_feedback_times > 0.0 * snii_th_feedback_times.units,
                        np.abs(snii_th_feedback_times.to("Myr") - snp_time_Myr)
                        < TIME_WINDOW_MYR,
                    )
                    mask_stars = np.logical_and(
                        snii_th_feedback_times_stars
                        > 0.0 * snii_th_feedback_times_stars.units,
                        np.abs(snii_th_feedback_times_stars.to("Myr") - snp_time_Myr)
                        < TIME_WINDOW_MYR,
                    )
                    ...
                    # Plot
                    ax.plot(
                        np.concatenate([centers, [centers[-1] + 1e-5]]),
                ...
                ax.legend(
                    fontsize=22, ncol=2, bbox_to_anchor=(-0.25, 1.25), loc="upper left"
                )
                ...
                ax.text(
                    ...
                    0.97,
                    "${:.1f} < t < {:.1f}$ Gyr".format(
                        (snp_time_Myr.value - TIME_WINDOW_MYR) / 1e3,
                        snp_time_Myr.value / 1e3,
                    ),
                    ha="right",
                    va="top",
